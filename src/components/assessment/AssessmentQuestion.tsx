...
interface AssessmentQuestionProps {
  question: {
    ...
    question_type?: string; // 'mcq' | 'text' | 'code' | 'video'
    options?: string[];
    points: number;
  };
...
export function AssessmentQuestion({ 
  ...
  // Parse options if it's a JSON string (common in Supabase for jsonb)
  const getOptions = () => {
  ...
  const getStarterCode = () => {
    // If options is an object with starterCode
    if (typeof question.options === 'string') {
        try {
            const parsed = JSON.parse(question.options);
            return parsed.starterCode || '';
        } catch {
            return '';
        }
    }
    ...
    if (question.options && typeof question.options === 'object' && !Array.isArray(question.options)) {
        return question.options.starterCode || '';
    }
    return '';
  };
        ...
        return (
          <div className="space-y-2">
            ...
            <Textarea
              ...
              value={selectedAnswer !== undefined ? selectedAnswer : getStarterCode()}
              onChange={(e) => onAnswerChange(e.target.value)}
            />
          </div>
        );
        ...
        return (
          <div className="space-y-4">
             <Alert>
              ...
              <AlertDescription>
                Video recording is simulated in this environment. Please describe what you would say in your video response.
              </AlertDescription>
            </Alert>
            <Textarea
              placeholder="[Simulation] Describe your video response..."
              className="min-h-[150px]"
              value={selectedAnswer || ''}
              onChange={(e) => onAnswerChange(e.target.value)}
            />
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-3">
            ...
            {question.question_type && (
                <Badge variant="secondary" className="uppercase text-xs">{question.question_type}</Badge>
            )}
          </div>
