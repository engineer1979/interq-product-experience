  ...
  const fetchAssessmentData = async () => {
    try {
      ...
      const classify = (q: any) => {
        if (q.points >= 7) return 'hard';
        if (q.points >= 4) return 'medium';
        return 'easy';
      };
      const byDifficulty = {
        easy: transformedQuestions.filter((q: any) => classify(q) === 'easy'),
        medium: transformedQuestions.filter((q: any) => classify(q) === 'medium'),
        hard: transformedQuestions.filter((q: any) => classify(q) === 'hard'),
      };
      const takeRandom = (arr: any[], count: number) => {
        const shuffled = [...arr].sort(() => Math.random() - 0.5);
        return shuffled.slice(0, Math.min(count, shuffled.length));
      };
      const selected = [
        ...takeRandom(byDifficulty.easy, 8),
        ...takeRandom(byDifficulty.medium, 8),
        ...takeRandom(byDifficulty.hard, 4),
      ].sort(() => Math.random() - 0.5);
      setQuestions(selected.length ? selected : transformedQuestions);
